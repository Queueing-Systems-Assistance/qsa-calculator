...
public class SystemMM1nonPreempCalculator {
    ...
    public double NAvg1(Map<SystemFeature, Double> features) {
        return Ro1(features) * (1 + Ro2(features)) / (1 - Ro1(features));
    }

    public double NAvg2(Map<SystemFeature, Double> features) {
        return Ro2(features) * (1 - Ro1(features) * (1 - Ro1(features) - Ro2(features))) / ((1 - Ro1(features)) * (1 - Ro1(features) - Ro2(features)));
    }
    ...
    public double PiWr(Map<SystemFeature, Double> features) {
        double result = TAvg(features)
                        * Math.log(CalculatorHelper.VALUE_100 * Ro(features)
                                   / (CalculatorHelper.VALUE_100 - features.get(r)));
        return result < 0 ? 0 : result;
    }
    ...
    public double TAvg2(Map<SystemFeature, Double> features) {
        return (1 - Ro1(features) * (1 - Ro1(features) - Ro2(features))) / (features.get(Mu) * (1 - Ro1(features)) * (1 - Ro1(features) - Ro2(features)));
    }
    ...
    public double WAvg1(Map<SystemFeature, Double> features) {
        return Ro2(features) * SAvg(features) + NAvg1(features) * SAvg(features);
    }
