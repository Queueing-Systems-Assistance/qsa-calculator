...
/**
 * System M | M | c | m | K Balking Service.
 */
@Component
public class SystemMMcmKBalkingCalculator extends SystemMMcmKBalkingRenegingAbstractCalculator {
    ...
    public double EW2(Map<SystemFeature, Double> features) {
        ...
        for (double i = c; i <= m - 1.0; i++) {
            ...
            sum += dividend * PinFin(PiiFeatures) / divisor;
        }
        return sum;
    }
    ...
    public double FWt(Map<SystemFeature, Double> features) {
        ...
        for (double i = c; i <= m - 1.0; i++) {
            ...
            PiiFeatures.put(SystemFeature.n, i);
            final double Pii = PinFin(PiiFeatures);
            sum += Pii * innerSum;
        }
        return 1 - sum;
    }
